...
- `_ds/mentorship/readme.md` — design system source notes and content/voice guidelines.
- `support.js` — runtime needed to render a Claude Design `.dc.html` canvas, if you create one
  here (e.g. `Dashboard.dc.html`, referencing `_ds/mentorship/` the same way onboarding's does).
...
- If you rename or restructure anything under `_ds/`, update every reference to that path in
  your `.dc.html` canvas and the files inside `_ds/mentorship/` (`_ds_manifest.json`,
