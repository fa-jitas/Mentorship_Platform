...
- `Mentorship Onboarding.dc.html` — the onboarding design canvas (Claude Design). Open it in a
  browser to view the onboarding screens; it pulls its styling from `_ds/mentorship/`.
- `_ds/mentorship/` — the Fly Bites design system: tokens (colors, type, spacing), components,
  and the UI kit the onboarding screens are built from. Shared with the dashboard team — see the
  root `CLAUDE.md`.
- `_ds/mentorship/readme.md` — design system source notes and content/voice guidelines (worth
...
- If you rename or restructure anything under `_ds/`, update every reference to that path in
  `Mentorship Onboarding.dc.html` and the files inside `_ds/mentorship/` (`_ds_manifest.json`,
